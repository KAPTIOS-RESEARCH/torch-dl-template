...
class BaseExperiment(AbstractExperiment):
    ...
    def __init__(self, config: dict):
        ...
        self.evaluator = self.load_evaluator(config['evaluator'])

        # LOGGER
        if self.config['track']:
            wandb.init(project="{{cookiecutter.project_name}}",
                       name=experiment_name, config=config, id=date_time, dir=self.log_dir)
